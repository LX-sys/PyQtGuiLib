...
# 动画类
class Animation2(AnimationAttr):
    Animation.Parallel = 1
    Animation.Sequential = 2
    '''
    ...
    # def blendStart(self):
    #     self.setAniMode(Animation2.Parallel)
    #
    # def blendEnd(self):
    #     self.setAniMode(Animation2.Sequential)

    def addAni(self,ani_data:dict):
        ani = AnimationABC(self.parent(), self.aniObjMode())
    ...
    # 添加连续动画(运动必须是相同的)
    def addSeriesAni(self, ani_data: dict, variation: list):
        ani_continuous = AnimationContinuous(self.parent(), self.aniObjMode())
        ani_continuous.setAniMode(AnimationContinuous.Sequential)
    ...
    def start(self):
        if self.aniMode() == Animation2.Parallel:
            self.setAniGroupObj(QParallelAnimationGroup(self.parent()))
        if self.aniMode() == Animation2.Sequential:
            self.setAniGroupObj(QSequentialAnimationGroup(self.parent()))
